...
1. `yarn version [patch | minor | major | the new version]`

2. git commit the results, typically:

   ```sh
   git add package.json .yarn/versions && git commit -m <the new version>
   ```

3. Use the [gitlab release feature](https://gitlab.com/isopod-cloud/chasm/-/releases) to create a new semver tag that matches the tag in step 1. Creating a new tag will trigger ci to publish both a npm package and docker image.
